export const translations = {
    'en-US': {
        title: 'Create Account',
        subtitle: 'Join the community of explorers today.',
        personalInfo: 'Personal Information',
        fullName: 'Full Name',
        fullNamePlaceholder: 'John Doe',
        email: 'Email Address',
        phone: 'Phone Number',
        nationality: 'Nationality',
        dob: 'Date of Birth',
        security: 'Security',
        password: 'Password',
        passwordPlaceholder: '******',
        confirmPassword: 'Confirm Password',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'Register',
        orSignup: 'or sign up with',
        alreadyAccount: 'Already have an account?',
        loginLink: 'Log In',
        passwordStrength: 'Password Strength:',
        weak: 'Weak',
        good: 'Good',
        veryGood: 'Very Good',
        errors: {
            required: 'is required',
            name: 'Please enter your first name and last name using valid letters.',
            email: 'Email is required',
            passwordLen: 'Password must be at least 6 chars',
            match: 'Passwords do not match',
            phoneLen: 'Phone number must be exactly 9 digits',
            terms: 'You must agree to the Terms and Conditions',
            captcha: 'Please verify the CAPTCHA'
        },
        otp: {
            title: 'Verify Your Account',
            subtitle: 'We sent a verification code to',
            label: 'Enter OTP Code',
            resend: 'Resend Code',
            changeEmail: 'Change Email',
            verifyBtn: 'Verify Account',
            continue: 'Continue with',
            resent: 'Code resent successfully!'
        },
        terms: {
            agree: 'I agree to the ',
            link: 'Terms and Conditions'
        }
    },
    'en-UK': {
        title: 'Create Account',
        subtitle: 'Join the community of explorers today.',
        personalInfo: 'Personal Information',
        fullName: 'Full Name',
        fullNamePlaceholder: 'John Doe',
        email: 'Email Address',
        phone: 'Phone Number',
        nationality: 'Nationality',
        dob: 'Date of Birth',
        security: 'Security',
        password: 'Password',
        passwordPlaceholder: '******',
        confirmPassword: 'Confirm Password',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'Register',
        orSignup: 'or sign up with',
        alreadyAccount: 'Already have an account?',
        loginLink: 'Log In',
        passwordStrength: 'Password Strength:',
        weak: 'Weak',
        good: 'Good',
        veryGood: 'Very Good',
        errors: {
            required: 'is required',
            name: 'Please enter your first name and last name using valid letters.',
            email: 'Email is required',
            passwordLen: 'Password must be at least 6 chars',
            match: 'Passwords do not match',
            phoneLen: 'Phone number must be exactly 9 digits',
            terms: 'You must agree to the Terms and Conditions',
            captcha: 'Please verify the CAPTCHA'
        },
        otp: {
            title: 'Verify Your Account',
            subtitle: 'We sent a verification code to',
            label: 'Enter OTP Code',
            resend: 'Resend Code',
            changeEmail: 'Change Email',
            verifyBtn: 'Verify Account',
            continue: 'Continue with',
            resent: 'Code resent successfully!'
        },
        terms: {
            agree: 'I agree to the ',
            link: 'Terms and Conditions'
        }
    },
    'pt': {
        title: 'Criar Conta',
        subtitle: 'Junte-se à comunidade de exploradores hoje.',
        personalInfo: 'Informações Pessoais',
        fullName: 'Nome Completo',
        fullNamePlaceholder: 'Nome Completo',
        email: 'Endereço de Email',
        phone: 'Número de Telefone',
        nationality: 'Nacionalidade',
        dob: 'Data de Nascimento',
        security: 'Segurança',
        password: 'Senha',
        passwordPlaceholder: '******',
        confirmPassword: 'Confirmar Senha',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'Registrar',
        orSignup: 'ou cadastre-se com',
        alreadyAccount: 'Já tem uma conta?',
        loginLink: 'Entrar',
        passwordStrength: 'Força da Senha:',
        weak: 'Fraca',
        good: 'Boa',
        veryGood: 'Muito Boa',
        errors: {
            required: 'é obrigatório',
            name: 'Por favor, insira seu nome e sobrenome usando letras válidas.',
            email: 'Email é obrigatório',
            passwordLen: 'A senha deve ter pelo menos 6 caracteres',
            match: 'As senhas não coincidem',
            phoneLen: 'O número de telefone deve ter exatamente 9 dígitos',
            terms: 'Você deve concordar com os Termos e Condições',
            captcha: 'Por favor, verifique o CAPTCHA'
        },
        steps: {
            step1: 'Informações Básicas',
            step2: 'Detalhes Pessoais',
            step3: 'Segurança'
        },
        buttons: {
            next: 'Próximo',
            back: 'Voltar',
            sendCode: 'Enviar Código'
        },
        forgotPassword: {
            title: 'Redefinir Senha',
            subtitle: 'Insira seu email ou nome de usuário para receber um código de verificação.',
            inputLabel: 'Email ou Nome de Usuário',
            success: 'Um código de verificação foi enviado para seu email.'
        },
        otp: {
            title: 'Verifique Sua Conta',
            subtitle: 'Enviamos um código de verificação para',
            label: 'Digite o código OTP',
            resend: 'Reenviar Código',
            changeEmail: 'Alterar Email',
            verifyBtn: 'Verificar Conta',
            continue: 'Continuar com',
            resent: 'Código reenviado com sucesso!'
        },
        terms: {
            agree: 'Eu concordo com os ',
            link: 'Termos e Condições'
        }
    },
    'es': {
        title: 'Crear Cuenta',
        subtitle: 'Únete a la comunidad de exploradores hoy.',
        personalInfo: 'Información Personal',
        fullName: 'Nombre Completo',
        fullNamePlaceholder: 'Nombre Completo',
        email: 'Correo Electrónico',
        phone: 'Número de Teléfono',
        nationality: 'Nacionalidad',
        dob: 'Fecha de Nacimiento',
        security: 'Seguridad',
        password: 'Contraseña',
        passwordPlaceholder: '******',
        confirmPassword: 'Confirmar Contraseña',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'Registrarse',
        orSignup: 'o regístrate con',
        alreadyAccount: '¿Ya tienes una cuenta?',
        loginLink: 'Iniciar Sesión',
        passwordStrength: 'Fortaleza:',
        weak: 'Débil',
        good: 'Buena',
        veryGood: 'Muy Buena',
        errors: {
            required: 'es obligatorio',
            name: 'Por favor ingrese su nombre y apellido usando letras válidas.',
            email: 'El correo es obligatorio',
            passwordLen: 'La contraseña debe tener al menos 6 caracteres',
            match: 'Las contraseñas no coinciden',
            phoneLen: 'El número de teléfono debe tener exactamente 9 dígitos',
            terms: 'Debes aceptar los Términos y Condiciones',
            captcha: 'Por favor verifica el CAPTCHA'
        },
        steps: {
            step1: 'Información Básica',
            step2: 'Detalles Personales',
            step3: 'Seguridad'
        },
        buttons: {
            next: 'Siguiente',
            back: 'Atrás',
            sendCode: 'Enviar Código'
        },
        forgotPassword: {
            title: 'Restablecer Contraseña',
            subtitle: 'Ingresa tu correo o nombre de usuario para recibir un código.',
            inputLabel: 'Correo o Usuario',
            success: 'Se ha enviado un código de verificación a tu correo.'
        },
        otp: {
            title: 'Verifica Tu Cuenta',
            subtitle: 'Enviamos un código de verificación a',
            label: 'Ingresa el código OTP',
            resend: 'Reenviar Código',
            changeEmail: 'Cambiar Email',
            verifyBtn: 'Verificar Cuenta',
            continue: 'Continuar con',
            resent: '¡Código reenviado con éxito!'
        },
        terms: {
            agree: 'Acepto los ',
            link: 'Términos y Condiciones'
        }
    },
    'fr': {
        title: 'Créer un Compte',
        subtitle: 'Rejoignez la communauté des explorateurs aujourd\'hui.',
        personalInfo: 'Informations Personnelles',
        fullName: 'Nom Complet',
        fullNamePlaceholder: 'Nom Complet',
        email: 'Adresse Email',
        phone: 'Numéro de Téléphone',
        nationality: 'Nationalité',
        dob: 'Date de Naissance',
        security: 'Sécurité',
        password: 'Mot de Passe',
        passwordPlaceholder: '******',
        confirmPassword: 'Confirmer le Mot de Passe',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'S\'inscrire',
        orSignup: 'ou inscrivez-vous avec',
        alreadyAccount: 'Vous avez déjà un compte ?',
        loginLink: 'Connexion',
        passwordStrength: 'Force:',
        weak: 'Faible',
        good: 'Bonne',
        veryGood: 'Très Bonne',
        errors: {
            required: 'est requis',
            name: 'Veuillez entrer votre prénom et nom en utilisant des lettres valides.',
            email: 'L\'email est requis',
            passwordLen: 'Le mot de passe doit contenir au moins 6 caractères',
            match: 'Les mots de passe ne correspondent pas',
            phoneLen: 'Le numéro de téléphone doit comporter exactement 9 chiffres',
            terms: 'Vous devez accepter les Termes et Conditions',
            captcha: 'Veuillez vérifier le CAPTCHA'
        },
        steps: {
            step1: 'Informations de Base',
            step2: 'Détails Personnels',
            step3: 'Sécurité'
        },
        buttons: {
            next: 'Suivant',
            back: 'Retour',
            sendCode: 'Envoyer le Code'
        },
        forgotPassword: {
            title: 'Réinitialiser le Mot de Passe',
            subtitle: 'Entrez votre email ou nom d\'utilisateur pour recevoir un code.',
            inputLabel: 'Email ou Nom d\'utilisateur',
            success: 'Un code de vérification a été envoyé à votre adresse email.'
        },
        otp: {
            title: 'Vérifiez Votre Compte',
            subtitle: 'Nous avons envoyé un code de vérification à',
            label: 'Entrez le code OTP',
            resend: 'Renvoyer le Code',
            changeEmail: 'Changer d\'Email',
            verifyBtn: 'Vérifier le Compte',
            continue: 'Continuer avec',
            resent: 'Code renvoyé avec succès !'
        },
        terms: {
            agree: 'J\'accepte les ',
            link: 'Termes et Conditions'
        }
    },
    'ru': {
        title: 'Создать Аккаунт',
        subtitle: 'Присоединяйтесь к сообществу исследователей сегодня.',
        personalInfo: 'Личная Информация',
        fullName: 'Полное Имя',
        fullNamePlaceholder: 'Полное Имя',
        email: 'Электронная Почта',
        phone: 'Номер Телефона',
        nationality: 'Гражданство',
        dob: 'Дата Рождения',
        security: 'Безопасность',
        password: 'Пароль',
        passwordPlaceholder: '******',
        confirmPassword: 'Подтвердите Пароль',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'Зарегистрироваться',
        orSignup: 'или зарегистрируйтесь через',
        alreadyAccount: 'Уже есть аккаунт?',
        loginLink: 'Войти',
        passwordStrength: 'Сложность:',
        weak: 'Слабый',
        good: 'Хороший',
        veryGood: 'Отличный',
        errors: {
            required: 'обязательно',
            name: 'Пожалуйста, введите имя и фамилию, используя допустимые буквы.',
            email: 'Email обязателен',
            passwordLen: 'Пароль должен содержать не менее 6 символов',
            match: 'Пароли не совпадают',
            phoneLen: 'Номер телефона должен содержать ровно 9 цифр',
            terms: 'Вы должны согласиться с Условиями и Положениями',
            captcha: 'Пожалуйста, проверьте CAPTCHA'
        },
        steps: {
            step1: 'Основная Информация',
            step2: 'Личные Данные',
            step3: 'Безопасность'
        },
        buttons: {
            next: 'Далее',
            back: 'Назад',
            sendCode: 'Отправить Код'
        },
        forgotPassword: {
            title: 'Сброс Пароля',
            subtitle: 'Введите email или имя пользователя для получения кода.',
            inputLabel: 'Email или Имя Пользователя',
            success: 'Код подтверждения был отправлен на ваш email.'
        },
        otp: {
            title: 'Подтвердите Свой Аккаунт',
            subtitle: 'Мы отправили код подтверждения на',
            label: 'Введите код OTP',
            resend: 'Отправить Код Повторно',
            changeEmail: 'Изменить Email',
            verifyBtn: 'Подтвердить Аккаунт',
            continue: 'Продолжить с',
            resent: 'Код успешно отправлен повторно!'
        },
        terms: {
            agree: 'Я согласен с ',
            link: 'Условиями и Положениями'
        }
    },
    'sw': {
        title: 'Unda Akaunti',
        subtitle: 'Jiunge na jumuiya ya wachunguzi leo.',
        personalInfo: 'Taarifa Binafsi',
        fullName: 'Jina Kamili',
        fullNamePlaceholder: 'Jina Kamili',
        email: 'Barua Pepe',
        phone: 'Nambari ya Simu',
        nationality: 'Uraia',
        dob: 'Tarehe ya Kuzaliwa',
        security: 'Usalama',
        password: 'Nenosiri',
        passwordPlaceholder: '******',
        confirmPassword: 'Thibitisha Nenosiri',
        confirmPasswordPlaceholder: '******',
        registerBtn: 'Jisajili',
        orSignup: 'au jisajili na',
        alreadyAccount: 'Je, una akaunti tayari?',
        loginLink: 'Ingia',
        passwordStrength: 'Nguvu:',
        weak: 'Dhaifu',
        good: 'Nzuri',
        veryGood: 'Nzuri Sana',
        errors: {
            required: 'inahitajika',
            name: 'Tafadhali ingiza jina la kwanza na la mwisho halali.',
            email: 'Barua pepe inahitajika',
            passwordLen: 'Nenosiri lazima liwe na angalau herufi 6',
            match: 'Manenosiri hayaendani',
            phoneLen: 'Nambari ya simu lazima iwe na tarakimu 9 haswa',
            terms: 'Lazima ukubali Sheria na Masharti',
            captcha: 'Tafadhali thibitisha CAPTCHA'
        },
        steps: {
            step1: 'Maelezo ya Kimsingi',
            step2: 'Maelezo ya Kibinafsi',
            step3: 'Usalama'
        },
        buttons: {
            next: 'Inayofuata',
            back: 'Nyuma',
            sendCode: 'Tuma Nambari'
        },
        forgotPassword: {
            title: 'Weka Upya Nenosiri',
            subtitle: 'Ingiza barua pepe au jina la mtumiaji ili kupokea nambari ya uthibitishaji.',
            inputLabel: 'Barua Pepe au Jina la Mtumiaji',
            success: 'Nambari ya uthibitishaji imetumwa kwa barua pepe yako.'
        },
        otp: {
            title: 'Thibitisha Akaunti Yako',
            subtitle: 'Tumetuma nambari ya uthibitishaji kwa',
            label: 'Ingiza Nambari ya OTP',
            resend: 'Tuma Tena Nambari',
            changeEmail: 'Badilisha Barua Pepe',
            verifyBtn: 'Thibitisha Akaunti',
            continue: 'Endelea na',
            resent: 'Nambari imetumwa tena kwa mafanikio!'
        },
        terms: {
            agree: 'Nakubali ',
            link: 'Sheria na Masharti'
        }
    }
};

export const languages = [
    { code: 'pt', name: 'Português', flag: '🇵🇹' },
    { code: 'en-US', name: 'English (US)', flag: '🇺🇸' },
    { code: 'en-UK', name: 'English (UK)', flag: '🇬🇧' },
    { code: 'fr', name: 'Français', flag: '🇫🇷' },
    { code: 'es', name: 'Español', flag: '🇪🇸' },
    { code: 'ru', name: 'Русский', flag: '🇷🇺' },
    { code: 'sw', name: 'Kiswahili', flag: '🇰🇪' },
];
